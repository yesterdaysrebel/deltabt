...
def main() -> int:
    ...
    if v == "PASS":
        ...
        w("")
        w("")
        w("### What the data DOES establish, precisely")
        w("")
        w("The verdict is INCONCLUSIVE because of Q1, not Q2. It would be wrong to")
        w("read it as \"adverse selection could not be measured\":")
        w("")
        w(f"    adverse selection @ +{h.PRIMARY_MARKOUT_MIN}m   {ac['mean']:+.3f} bps")
        w(f"    95% CI                     [{ac['ci_low']:+.3f}, {ac['ci_high']:+.3f}]")
        w(f"    MDE                         {ac['mde']:.3f} bps")
        w(f"    kill threshold              {KILL} bps")
        w(f"    CI upper is                 {KILL / ac['ci_high']:.1f}x BELOW the threshold")
        w("")
        w(f"That is a tight measurement on {ac['n']:,} fills across {ac['n_clusters']} clusters —")
        w("the MDE is under a basis point. Adverse selection on filled passive")
        w("orders is small, and nothing in this data suggests otherwise.")
        w("")
        w("**The binding limitation is the fill rate.** At "
          f"{cs['fill_rate']:.1%}, {cs['orders']:,} orders")
        w(f"produced only {cs['fills']:,} fills against a frozen target of {t['fills']:,}.")
        w("")
        w("### The queue ambiguity turned out not to matter")
        w("")
        w("The pre-registration warned that cancellations cannot be attributed to a")
        w("queue position, and bounded the fill rate rather than estimating it. On")
        w("this data the bound is real but non-binding: the two models disagree on")
        w("queue depth for **241 of 840 orders**, and on the fill outcome for")
        w("**zero**. Unfilled orders sit a median **3,882 contracts** short of")
        w("clearing — a gap no plausible cancellation attribution closes.")
        w("")
        w("So the one thing the feed could not resolve does not affect the answer.")
        w("")
