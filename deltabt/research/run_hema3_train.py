"""H-EMA-3 TRAIN. VALID IS NOT CONSTRUCTED IN THIS FILE.

    PYTHONPATH=. python3 -u -m deltabt.research.run_hema3_train
"""
...
def main() -> int:
    print("=" * 100)
    print("H-EMA-3 TRAIN -- paired mirror-direction barrier test")
    ...
    rows += report(pooled[pooled.entry_time < mid], "TRAIN-H1", half="H1")
    rows += report(pooled[pooled.entry_time >= mid], "TRAIN-H2", half="H2")
    ...
    res.to_csv(OUT / "train_results.csv", index=False)
    json.dump(rows, open(OUT / "train_results.json", "w"), indent=2, default=str)
    pooled.to_parquet(OUT / "bets_train.parquet", index=False)
