"""H-COST-1 Stage A + B on TRAIN. VALID IS NOT CONSTRUCTED IN THIS FILE."""
...
def main() -> int:
    ...
    print("=" * 104)
    print("H-COST-1 STAGE A -- timeframe x stop width x volatility, baseline cost, k=0.50")
    print(f"  prereg sha256 {MAN['preregistration_sha256'][:16]}...   "
          f"primary widths <=5%, diagnostic >5% OUT-OF-MODEL")
    print("=" * 104)
    t0 = time.time()
    bets_all = pd.read_parquet("out/hema3/bets_train.parquet")
    ...
    json.dump(cmeta, open(OUT / "summaries" / "conflict_report.json", "w"), indent=2)
    ...
    for tf in MAN["exec_timeframes"]:
        ...
        for s in data:
            ...
            pool.append(v[m])
        thr[tf] = hcost1.volatility_thresholds(np.concatenate(pool))
        print(f"  vol thresholds {tf:>3}m: P33 {thr[tf][0]*100:.4f}%  P67 {thr[tf][1]*100:.4f}%")
    json.dump({str(k): v for k, v in thr.items()},
              open(OUT / "summaries" / "volatility_thresholds.json", "w"), indent=2)
    ...
    for df, nm in ((a, "feasibility_map"), (b, "cost_sensitivity")):
        df.to_csv(OUT / "summaries" / f"{nm}.csv", index=False)
    json.dump(dict(stage_a=rows_a, stage_b=rows_b),
              open(OUT / "train_results.json", "w"), indent=2, default=str)
    ...
    prim["gate"] = [hcost1.gate(r._asdict(), r.net_low_slip, r.net_high_slip)
                    for r in prim.itertuples()]
    prim.to_csv(OUT / "summaries" / "stop_width_summary.csv", index=False)
