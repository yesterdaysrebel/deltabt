...
def test_the_live_identity_file_really_holds_the_identity():
    ...
    # WHERE the credential lives is part of this file's identity too, even
    # though Terraform neither creates nor reads it: the host resolves the
    # secret by this name, so changing it changes what the bot authenticates
    # with.
    assert "live_credential_name" in text
