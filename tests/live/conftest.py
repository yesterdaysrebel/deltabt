...
async def pg_repo():
    ...
    async with repo._pool.acquire() as con:
        await con.execute(
            "TRUNCATE paper_fills, paper_orders, positions, strategy_signals, "
            "risk_events, system_events, market_candles, heartbeat, "
            "bot_instance, strategy_state RESTART IDENTITY CASCADE")
    try:
        yield repo
    finally:
        await repo.close()
